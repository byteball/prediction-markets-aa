...
describe('Check tokenless prediction AA with draw (base)', function () {
	this.timeout(120000)
	...
	before(async () => {
		this.network = await Network.create()
			...
			.with.wallet({ bob: { base: 50e9, reserveAsset: 50e9 } })
	...
	it('Alice issue tokens (not enough reserve)', async () => {
		...
		expect(response.bounced).to.be.false;
		expect(response.response.responseVars.error).to.equal(`expected reserve amount: ${Math.abs(res.reserve_needed + res.fee + 1e4)}`);
	});
	...
	it('Bob issues tokens by type (no)', async () => {
		const res = this.get_result_for_buying_by_type('no', 5e9);
	...
	it('Bob issues tokens by type (draw)', async () => {
		...
		expect(vars1[`balance_${this.bobAddress}`].draw).to.be.equal(this.bob_draw_amount + res.amount);
	...
	it('Alice claims profit again with only losing tokens (bounced)', async () => {
